...
-- xml version:
-- 88340 rows
-- (or 86112 if adding "HAVING parentLocation IS NOT NULL" clause)


				INSERT INTO `periodicallocationmatches_xml_implementation`
				  (recordId, title, parentRecordId, parentLocation, parentTitle)
				...
				LEFT JOIN periodicallocations ON catalogue_xml.matchTitle = periodicallocations.title
				...
				AND matchTitle IS NOT NULL
			;

-- flat version:
-- EXACT MATCH BY DIFF CSV COMPARISON
-- 88340 rows:

				INSERT INTO `periodicallocationmatches_flat_implementation`
				  (recordId, title, parentRecordId, parentLocation, parentTitle)
				SELECT
					child.recordId,
					self.value,
					...
					periodicallocations.title AS parentTitle	-- Necessary to enable HAVING, but useful for debugging anyway
				FROM catalogue_processed AS child
				JOIN catalogue_processed AS self ON child.recordId = self.recordId AND self.xPath = '/art/j/tg/t'
				LEFT JOIN periodicallocations ON self.value = BINARY periodicallocations.title
				LEFT JOIN catalogue_processed AS parent ON periodicallocations.recordId = parent.recordId AND parent.field = 'Location'
				WHERE child.field = 'location' AND child.value = 'Periodical'
				ORDER BY recordId
			;

-- flat version, using new (corrected) grouped locations
-- 82878 rows - and /records/1192/ no longer split into two
** WORKING @ 79988: **
				INSERT INTO `periodicallocationmatches_flat`
				SELECT
					...
					'/art/j/tg/t' AS matchTitleField
				FROM catalogue_processed AS child
				LEFT JOIN catalogue_processed AS selfTitle ON child.recordId = selfTitle.recordId AND selfTitle.xPath = '/art/j/tg/t'
				LEFT JOIN periodicallocations ON selfTitle.value = BINARY periodicallocations.title
				WHERE child.field = 'location' AND child.value = 'Periodical'
				HAVING value != ''	-- Using this gives 82185 results
				-- HAVING periodicallocations.location IS NOT NULL	-- Using this gives 79988 results
				ORDER BY recordId
			;
...
-- xml version:
-- 315 rows
-- 280 rows if surrounding with select distinct recordId from ( ... ) AS origin;

				INSERT INTO `periodicallocationmatches2_xml_implementation`
				  (recordId, title, parentRecordId, parentLocation, parentTitle)
				SELECT
					child.recordId,
					catalogue_xml.matchTitle AS title,
				...
				FROM catalogue_processed AS child
				LEFT JOIN catalogue_xml ON child.recordId = catalogue_xml.id
				LEFT JOIN periodicallocations ON catalogue_xml.matchTitle LIKE CONCAT(periodicallocations.title, '%')
				WHERE child.field = 'location' AND child.value = 'Periodical'
...
-- flat version:
-- 350 RECORDS, SO SOME EXTRA, AND ALSO NEED TO CHECK DATA EXACTLY MATCHES

				INSERT INTO `periodicallocationmatches2_flat_implementation`
				  (recordId, title, parentRecordId, parentLocation, parentTitle)
				SELECT
					child.recordId,
					self.value,
					...
					periodicallocations.title AS parentTitle	-- Necessary to enable HAVING, but useful for debugging anyway
				FROM catalogue_processed AS child
				JOIN catalogue_processed AS self ON child.recordId = self.recordId AND self.xPath = '/doc/ts'
				LEFT JOIN periodicallocations ON CONCAT(self.value, '%') LIKE BINARY periodicallocations.title AND periodicallocations.title IS NOT NULL
				LEFT JOIN catalogue_processed AS parent ON periodicallocations.recordId = parent.recordId AND parent.field = 'Location'
				WHERE child.field = 'location' AND child.value = 'Periodical'
				ORDER BY recordId
			;
				...
				FROM catalogue_processed AS child
				LEFT JOIN catalogue_xml                    ON child.recordId = catalogue_xml.id
				LEFT JOIN catalogue_processed AS selfTitle ON child.recordId = selfTitle.recordId AND selfTitle.xPath = '/doc/ts'
				LEFT JOIN periodicallocations ON catalogue_xml.matchTitle LIKE CONCAT(periodicallocations.title, '%')
				WHERE child.field = 'location' AND child.value = 'Periodical'
				AND LENGTH(EXTRACTVALUE(xml, '/doc/ts[1]')) > 0
				HAVING periodicallocations.title IS NOT NULL	-- Necessary to strip out LEFT JOIN non-matches; INNER JOIN is too slow
...
This gives 305, uniqued to 280:
				...
				WHERE child.field = 'location' AND child.value = 'Periodical'
				AND LENGTH(EXTRACTVALUE(xml, '/doc/ts[1]')) > 0
				HAVING periodicallocations.title IS NOT NULL
				ORDER BY recordId
-> due to the partial match - the extras are duplicates


but this gives 280, uniqued (to 280):
removing the HAVING puts it up to 294
...
** WORKING @ 280/294: **
				...
				SELECT
					NULL,	-- Auto-populate auto-increment field
					child.recordId,
					SUBSTRING_INDEX(selfTitle.value,' ; ', 1) AS value,
					periodicallocations.recordId AS parentRecordId,
					periodicallocations.location AS parentLocation,
					periodicallocations.title AS parentTitle,
					'/doc/ts[1]' AS matchTitleField
				FROM catalogue_processed AS child
				LEFT JOIN catalogue_processed AS selfTitle ON child.recordId = selfTitle.recordId AND selfTitle.xPathWithIndex = '/doc/ts[1]'
				LEFT JOIN periodicallocations ON SUBSTRING_INDEX(selfTitle.value,' ; ', 1) = BINARY periodicallocations.title
				WHERE child.field = 'location' AND child.value = 'Periodical'
				HAVING value IS NOT NULL	-- Using this gives 294 results
				-- HAVING periodicallocations.title IS NOT NULL	-- Using this gives 280 results
				ORDER BY recordId
			;
...
explain
	SELECT catalogue_processed.recordId, line, field, value, xPath
	FROM catalogue_processed
	JOIN (
		SELECT
			recordId,
		...
		INNER JOIN catalogue_processed ON
		    filtered.recordId = catalogue_processed.recordId
		AND filtered.minLine  = catalogue_processed.line
    WHERE xPath = '/doc/ts'
;
...
explain
	SELECT catalogue_processed.*
	FROM catalogue_processed
	LEFT JOIN catalogue_processed AS filtered ON catalogue_processed.recordId = filtered.recordId AND catalogue_processed.line < filtered.line
	WHERE filtered.recordId IS NULL
	AND xPath = '/doc/ts'
