...
class muscatConversion extends frontControllerApplication
{
	...
	public function databaseStructure ()
	{
		return "
			...
			) ENGINE=MyISAM  DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='MARC parser definition';
	...
	private function presentedRecord ($record)
	{
		...
		# Location
		$table['Location(s)'] = false;
		if (isSet ($record['852'])) {
			$supportedSubfields = array ('b', 'c', 'h');
			$locations = array ();
			foreach ($record['852'] as $index => $record852) {
				...
				foreach ($supportedSubfields as $supportedSubfield) {
					if (isSet ($record852['subfields'][$supportedSubfield])) {
						...
						if ($locationComponent == 'SPRI-NIS') {
							...
							if (isSet ($record852['subfields']['x'])) {		// $x is optional, and is only used if there is another (i.e. non- SPRI-NIS) location
				...
				if ($locationComponents) {
					$locations[] = implode (': ', $locationComponents);
				}
			}
			if ($locations) {
				$table['Location(s)'] = implode ('<br />', $locations);
			}
		}
	...
	public function reportdownload ($id)
	{
		...
		$query = "
			SELECT
				...
				ExtractValue(xml, '//loc[1]/location') AS 'Location (1)',
				ExtractValue(xml, '//loc[2]/location') AS 'Location (2)',
				ExtractValue(xml, '//loc[3]/location') AS 'Location (3)',
				ExtractValue(xml, '//kg[1]') AS '*kg (1)',
				ExtractValue(xml, '//kg[2]') AS '*kg (2)',
