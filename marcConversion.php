...
class marcConversion
{
	...
	private function macro_generate300 ($value_ignored, $parameter_ignored = false, &$errorHtml)
	{
		...
		if (!strlen ($result) || in_array (strtolower ($this->pOrPt['citation']), array ('unpaged', '[unpaged]'))) {	 // 'unpaged' at /records/1248/ (test #341); 'Unpaged' at /records/174009/ (test #343); '[unpaged]' at /records/24258/ (test #987)
			...
			} else if (in_array ($this->form, array ('Microfiche', 'Microfilm'))) {
				$result = '1 microform';	// E.g. /records/60971/ (test #989)
			} else {
	...
	private function macro_languages041 ($value_ignored, $indicatorMode = false, &$errorHtml)
	{
		...
		foreach ($strings as $note) {
			...
			# Perform a match, e.g. /records/4353/ (test #372), /records/13398/ (test #1091); NB this matching does not use a starting at (^) match e.g. /records/190904/ which starts "English translation from Russian" (test #377)
			if (
				   preg_match ('/[Tt]ranslat(?:ion|ed) (?:from|reprint of)(?: original| the original| the|) ([a-zA-Z]+)/i', $note, $matches)
				|| preg_match ('/[Tt]ranslation of (Danish|English|Eskimo|Finnish|French|German|Norwegian|Russian|Swedish) original\b/', $note, $matches)	// E.g. /records/13398/ (test #1091
			) {	// Deliberately not using strip_tags, as that would pick up Translation from <em>publicationname</em> which would not be wanted anyway, e.g. /records/8814/ (test #378)
	...
	private function macro_generateLeader ($value)
	{
		...
		#!#M If merging, we would need to have a check that this matches
		switch ($this->form) {
		...
		#!#M If merging, we would probably overwrite whatever is currently present in Voyager as 00000, so the computer re-computes it
		$string .= '00000';
		...
		#!#M If merging, we think that # is better than 7; other values would need to be checked; NB the value '7' could be a useful means to determine Voyager records that are minimal (i.e. of limited use)
		$string .= '#';
		
		# Position 18: Descriptive cataloguing form
		#!#M If merging, we would need to check with the UL that our 'a' trumps '#'; other values would need to be checked
		$string .= 'a';	// Denotes AACR2
		
		# Position 19: Multipart resource record level
		#!#M If merging, we need to check that our '#' is equivalent to ' ' in Voyager syntax
	...
	private function macro_generate007 ($value)
	{
		...
		# Look up the value, looping through each *form
	...
	private function macro_generate500analytics ($value, $parameter_unused)
	{
		...
		# Add dot at end, e.g. /art/j genuine analytic: /records/1166/ (test #984); /art/j pseudo-analytic: /records/214872/ (test #595); /art/in genuine analytic (which will exist because generate245 does this): /records/1166/ (test #985); /art/in pseudo-analytic: /records/1244/ added (test #593), /records/1107/ already present (test #594); see: https://www.oclc.org/bibformats/en/specialcataloging.html#CHDEBCCB
	...
	private function filterTokenCreation ($locations /* array of locations (if any) or single location or false */)
	{
		...
		foreach ($locations as $location) {
			...
			$locationMatches = array (
				...
				'Periodical'							=> 'SUPPRESS-MISSINGLOCATIONPERIODICAL',	// 151 records, as in /reports/locationperiodical/ , e.g. /records/23057/ (test #1088, #1089)
