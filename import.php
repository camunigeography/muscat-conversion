...
class import
{
	...
	private function createFieldsindexTable ()
	{
		...
		# 	ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci	/* MyISAM forced, so that FULLTEXT search can be used */
		# 	AS
		# 	(SELECT
		# 		recordId AS id,
	...
	*/
	private function createTransliterationsTable ()
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS `transliterations` (
			...
			PRIMARY KEY (`id`),
			INDEX(`field`)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Table of transliterations'
		;";
		$this->databaseConnection->execute ($sql);
		...
		$sql = "
			CREATE TABLE IF NOT EXISTS tickednames (
				...
				PRIMARY KEY (id),
				INDEX(surname)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Table of names ticked during manual checking';
		";
		$this->databaseConnection->execute ($sql);
	...
	private function processXmlRecords ($pathSeedingOnly = false, &$errorsHtml)
	{
		...
		if ($pathSeedingOnly) {
			...
			$sql = "CREATE TABLE IF NOT EXISTS `catalogue_processed_xpaths_temp` (
				...
				PRIMARY KEY (id)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Temporary table of xPaths for joining'
			;";
			$this->databaseConnection->execute ($sql);
		}
	...
	private function processPeriodicalLocations (&$errorsHtml)
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS `periodicallocations` (
			...
			INDEX(recordId),
			INDEX(title)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Table of periodical locations'
		;";
		$this->databaseConnection->execute ($sql);
		...
		$sql = "CREATE TABLE IF NOT EXISTS `periodicallocationmatches` (
			...
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Table of periodical location matches'
		;";
		$this->databaseConnection->execute ($sql);
		...
		#!# Potentially the BINARY match restriction should be relaxed, so that it is case-insensitive, e.g. "Annales Zoologici Societatis Zoologicae-Botanicae Fennicae Vanamo" (/records/15495/) vs "Annales zoologici Societatis zoologicae-botanicae Fennicae Vanamo" (/records/214736/) though this matches on *kg anyway; maybe find using query based on: `SELECT * FROM catalogue_processed WHERE field = 't' and xPath LIKE '/ser/%' AND value IN( SELECT DISTINCT LOWER(value) AS value FROM catalogue_processed WHERE field = 't' AND xPath like '/art/%' AND recordId IN(SELECT recordId FROM `catalogue_processed` WHERE `field` LIKE 'location' AND `value` LIKE 'Periodical') );`
		$groupings = array (
	...
	public function createVolumeNumbersTable ()
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS volumenumbers (
			...
			PRIMARY KEY (id)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Table of volume numbers'
		;";
		$this->databaseConnection->execute ($sql);
	...
	private function runReports ()
	{
		...
		$query = "CREATE TABLE IF NOT EXISTS reportresults (
			...
			PRIMARY KEY (`id`),
			KEY `report` (`report`)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Results table' AUTO_INCREMENT=1 ;
		";
		$this->databaseConnection->query ($query);
	...
	public function runTests (&$errorHtml = false, $regenerateMarc = false, $importMode = true)
	{
		...
		$sql = "
			CREATE TABLE `tests` (
			  ...
			  PRIMARY KEY (`id`)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Test results';";
		$this->databaseConnection->execute ($sql);
